...
| [ADR-CorpusOutsideThePackage](ADR-CorpusOutsideThePackage.md) | The repository is a workspace -- core publishes, validation holds the tests with no mirror, fixture is the corpus outside both -- because cargo drops any subdirectory holding a Cargo.toml, so a corpus inside the package shipped 64 tests and none of what they read. |
