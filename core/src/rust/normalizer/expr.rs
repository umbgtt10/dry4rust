...
pub fn normalize_expr(expr: &Expr, ctx: &mut NormalizationContext) -> NormalizedNode {
    match expr {
        Expr::Lit(el) => normalize_lit(&el.lit),
        Expr::Path(ep) => {
        ...
        // BinaryOp -> [left, right]
        Expr::Binary(eb) => NormalizedNode::with_children(
        ...
        // UnaryOp -> [operand]
        Expr::Unary(eu) => NormalizedNode::with_children(
        ...
        // Index -> [base, index]
        Expr::Index(ei) => NormalizedNode::with_children(
            NodeKind::Index,
            vec![
        ...
        Expr::Continue(_) => NormalizedNode::leaf(NodeKind::Continue),
        // Assign -> [left, right]
        Expr::Assign(ea) => NormalizedNode::with_children(
            NodeKind::Assign,
            vec![
        ...
        // Reference -> [expr]
        Expr::Reference(er) => NormalizedNode::with_children(
            NodeKind::Reference {
                mutable: er.mutability.is_some(),
            },
            vec![normalize_expr(&er.expr, ctx)],
        ),
        Expr::Tuple(et) => NormalizedNode::with_children(
            NodeKind::Tuple,
            et.elems.iter().map(|e| normalize_expr(e, ctx)).collect(),
        ),
        Expr::Array(ea) => NormalizedNode::with_children(
            NodeKind::Array,
            ea.elems.iter().map(|e| normalize_expr(e, ctx)).collect(),
        ),
        // Repeat -> [elem, len]
        Expr::Repeat(er) => NormalizedNode::with_children(
            NodeKind::Repeat,
            vec![normalize_expr(&er.expr, ctx), normalize_expr(&er.len, ctx)],
        ),
        // Cast -> [expr, ty]
        Expr::Cast(ec) => NormalizedNode::with_children(
            NodeKind::Cast,
        ...
        Expr::If(ei) => NormalizedNode::with_children(
            NodeKind::If,
            vec![
        ...
        Expr::While(ew) => NormalizedNode::with_children(
            NodeKind::While,
            vec![
        ...
        Expr::ForLoop(ef) => NormalizedNode::with_children(
            NodeKind::ForLoop,
            vec![
        ...
        // LetExpr -> [pat, expr]
        Expr::Let(el) => NormalizedNode::with_children(
            NodeKind::LetExpr,
            vec![normalize_pat(&el.pat, ctx), normalize_expr(&el.expr, ctx)],
        ),
        Expr::Macro(em) => normalize_macro(&em.mac, ctx),
        Expr::Group(eg) => normalize_expr(&eg.expr, ctx),
        Expr::Unsafe(eu) => normalize_block(&eu.block, ctx),
        Expr::Const(ec) => normalize_block(&ec.block, ctx),
        _ => NormalizedNode::leaf(NodeKind::Opaque),
    }
}

pub fn normalize_stmt(stmt: &Stmt, ctx: &mut NormalizationContext) -> NormalizedNode {
    match stmt {
        // LetBinding -> [pattern, type_or_None, init_or_None, diverge_or_None]
        Stmt::Local(local) => NormalizedNode::with_children(
            NodeKind::LetBinding,
            vec![
        ...
        Stmt::Item(_) => NormalizedNode::leaf(NodeKind::Opaque),
        Stmt::Macro(sm) => {
...
pub fn normalize_block(block: &Block, ctx: &mut NormalizationContext) -> NormalizedNode {
    NormalizedNode::with_children(
        NodeKind::Block,
